...

{
  ...
  programs.niri.settings = {
    ...
    layout = {
      ...
      # ─── Focus ring (Gruvbox Soft Dark) ──────────────────────────────
      focus-ring = {
        enable = true;
        width = 2;
        active.color = "#8ec07c";          # aqua
        inactive.color = "#504945";        # bg2
      };

      # Border desligado (focus-ring já indica + border ocupa espaço útil).
      border.enable = false;

      # Background da workspace.
    ...
    binds = with config.lib.niri.actions; {
      ...
      # ─── Screenshots ──────────────────────────────────────────────────
      # # ─── Screenshots com Swappy ──────────────────────────────────────────
      "Print".action.spawn = [ "sh" "-c" "grim -g \"$(slurp)\" - | swappy -f -" ];
      "Ctrl+Print".action.spawn = [ "sh" "-c" "grim - | swappy -f -" ];
      "Alt+Print".action.spawn = [ "sh" "-c" "niri msg action screenshot-window | swappy -f -" ];
      #"Print".action            = "screenshot";             # interativo (área)
      #"Ctrl+Print".action       = screenshot-screen;      # tela inteira
      #"Alt+Print".action        = screenshot-window;      # janela ativa
      ...
      # ─── Lock / Power ──────────────────────────────────────────────────
      "Mod+Alt+L" = {
        action = spawn "swaylock" "-f" "-c" "282828";
        allow-when-locked = true;
      };
