...

{
  programs.niri.settings = {
    ...
    # Confirme o nome com `niri msg outputs` após instalar.
    # mode omitido propositalmente — evita mismatch de refresh rate.
    outputs."eDP-1" = {
      scale = 1.0;
    ...
    binds = with config.lib.niri.actions; {
      ...
      # workaround issue #944 do niri-flake
      ...
      # ─── Sistema ───────────────────────────────────────────────────────────
      "Mod+Alt+L"   = { action = spawn "swaylock" "-f" "-c" "1b2430"; allow-when-locked = true; };
